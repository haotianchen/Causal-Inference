...
#### Disadvantages: 
 ...
 - Abadie-Imbens bias adjustment: $$\hat{\tau}_{ATT}  = \frac{1}{N_1} \sum_{D_i=1} [Y_i - Y_{j(i)} - (\hat \mu_0(X_i) - \hat \mu_0(X_{j(i)}))]$$ where $\mu(x) = \E[Y|X, D=0]$. In R, use `Match(Y, Tr, X, BiasAdjust = TRUE)`
...
### Idea
 ...
 - Motivation: the curse of dimensionality makes it difficult to work in a situation where there are many (pre-treatment) covariates to condition on.
 ...
 - Assuming conditional ignorability and common support, we have $\{ Y_{0i}, Y_{1i} \} \independent D_i|\pi_i$, which is called the Balancing Property of the propensity score.
...
### Coding Example
First, let's calculate the propensity scores using logistic regression.
