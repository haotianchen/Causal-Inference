...
Let's assume you are Doctor Strange --- i.e., you can observe what's happening in the multiverses. 
...
Actually, we can rewrite the ATE as a weighted average: ATE = $Pr(D_i=1)$ ATT + $Pr(D_i=0)$ ATC (**hint**: related to Q2, Pset 2). Here you can consider $Pr(D_i=1)$ as the probability/proportion treated. 
...
### RI with larger data
  ...
  - Now let's play with R code. **Hint**: this relates to Q4, Pset2.
```{r}
set.seed(1234)
...
ri_function <- function(Y, D, M){
  ...
  #2. create potential outcomes under sharpnull
  Y0_sn = Y
  Y1_sn = Y
